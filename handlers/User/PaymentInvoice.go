...
func PymentInvoice(c *gin.Context) {
	...
	orderID := c.Param("ID")
	if err := initializers.DB.Where("order_id = ?", orderID).Preload("Product").Preload("Order.Address").Find(&orderItems); err.Error != nil {
		c.JSON(500, gin.H{
	...
	for _, address := range orderItems {
		// date := address.Order.OrderDate.Format("2006-01-02")
	...
	for _, invoice := range orderItems {
		...
		Discount = GrandTotal - invoice.Order.OrderPrice
